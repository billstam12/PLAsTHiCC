...
def iter_time_serie_by_id(file, chunk_size = 10000,max_n=sys.maxsize):
    ...
    for l in reader:
        ...
        if len(chunk) >= chunk_size:
            yield (np.array(chunk), np.array(ids,dtype=np.int32))
            chunk = list()
            ids = list()
            counter += len(chunk)
        if counter > max_n:
...
def tailored_loss(true_output, loss_input):
    n = K.sum(true_output[:,2,:,:])
    flux_diff = (loss_input[:,0,:,:] - true_output[:,0,:,:])
    flux_err_diff = (loss_input[:,1,:,:] - true_output[:,1,:,:])
    flux_loss = K.sum(flux_diff * flux_diff * true_output[:,2,:,:]) / n
    flux_err_loss = K.sum(flux_err_diff * flux_err_diff * true_output[:,2,:,:]) / n
